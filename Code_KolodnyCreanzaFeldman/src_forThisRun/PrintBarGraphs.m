% This is a script that has a lot of junk in it that can be ignored: archaeological remains of the previous uses/versions and things that are relevant only in certain context, potentially only partially related.

% function evolutioninleapsNCresults
close all
...
% actual run params:
N = 100;
P_l = 0.002; % lucky leap
selection_lambda = 10;
P_s = 0.001 ; % spontanous loss
l = 10.5 ;% mean toolkit size
f = 0.2;


s = 1/selection_lambda;
P_lc = P_l*s; % P_l tag == corrected for selection-coefficient loss
P_lct = P_lc * (l+1); % P lucky (corrected for s) + inclusion of toolkit
P_sc = P_s * (2*l+1)/(l+1); % loss corrected for loss of whole toolkits
...
cd('/Users/orenkolodny/Documents/workspace/CulturalDiv8_ForAddendum/output/')
d=dir('/Users/orenkolodny/Documents/workspace/CulturalDiv8_ForAddendum/output/');
d(1:3)=[];
...
for i=0:0 %increase this if number of iterations in main.java increases
    
    
    %change "2popsOfSize500" if popsize or #pops changes
    filename = sprintf('%s%d%s','run',i,'/NumOfToolsPerGeneration.txt');
%     filename = sprintf('%s%d%s','run',i,'_1popsOfSize100Type0/NumOfToolsPerGeneration.txt');
   filename
    M=csvread(filename);
...
%xlim([23000 24000]); ylim([800 1200])